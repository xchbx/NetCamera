...
private:
	...
	{
	public:
		MIPConnection(MIPComponent *pPull, MIPComponent *pPush, bool feedback, uint32_t mask1,uint32_t mask2)
		{ 
			m_mask1 = mask1; 
			m_mask2 = mask2; 
			m_pPull = pPull; 
		...
		bool operator==(const MIPConnection &c)	const
		{ 
			if (m_pPull == c.m_pPull && m_pPush == c.m_pPush \
			 && m_mask1 == c.m_mask1 && m_mask2 == c.m_mask2 \
			 && m_feedback == c.m_feedback)
			 {
				return true;
			 }
			return false;
